...
'''
seg_data_files = glob('seg_stat_saves/*.csv')

regressors = []

for s_ds in seg_data_files:

	print(int(s_ds[15:17]))

	dataset = []

	with open(s_ds, 'rU') as fn:
	...
	regressors.append([int(s_ds[15:17]), regressor])

with open('regressors_1.pickle', 'wb') as handle:
...
plt.figure()
plt.imshow(average_heat_map, cmap = plt.cm.jet, vmin=0, vmax=1) # vmax=0.6
plt.axis('off') 
